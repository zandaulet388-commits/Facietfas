...
TOKEN = "8818405158:AAHi_tdbdhF4WVd9ujj75EXtsji-jyJNvyc"
...
conn = sqlite3.connect("StandVENSEU.db", check_same_thread=False)
cur = conn.cursor()
...
mode_device = {
    ...
    "mobile_1x1": "📱 Лобби (MOBILE)",
    "mobile_2x2": "📱 Лобби (MOBILE)",
    "mobile_5x5": "📱 Лобби (MOBILE)"
}
...
bot.send_message(  
    message.chat.id,  
    "🛡 Добро пожаловать в StandVENSEU Faciet\n\n"  
    "🎮 Введи ник в игре"  
)  
...
if not users:  
    bot.send_message(message.chat.id, "🏆 ТОП ELO пуст")  
    return  
...
for nickname, elo in users:  

    # защита от мусора  
    if not nickname:  
        continue  

    text += f"{pos}. {nickname} — {elo}\n"  
    pos += 1  
...
# FIX + 5v5 + ELO BALANCE + CAPTAINS PICK/BAN

=========================================

captains = {}

ROLES_5V5 = ["IGL", "ENTRY", "AWP", "SUPPORT", "LURKER"]
...
# сортируем по ELO
players = sorted(players, key=lambda x: get_user(x)[4] if get_user(x) else 0, reverse=True)
...
# если перекос по ELO — меняем местами слабых/сильных  
for _ in range(10):  
    diff = calc_team_power(team1) - calc_team_power(team2)  

    if abs(diff) < 200:  
        break  

    if diff > 0:  
        team1.sort(key=lambda x: get_user(x)[4])  
        team2.sort(key=lambda x: get_user(x)[4], reverse=True)  
    else:  
        team1.sort(key=lambda x: get_user(x)[4], reverse=True)  
        team2.sort(key=lambda x: get_user(x)[4])  
...
role = ROLES_5V5[i % len(ROLES_5V5)]
result[uid] = role
return result
...
def start_match(mode):
    ...
    players = lobbies.get(mode, [])

    need_players = sizes[mode]

    if len(players) < need_players:
        return 

# 1x1
if need_players == 2:

    text = f"🛡 MATCH STARTED ({mode})\n\n"
...
# 5x5
    ...
    roles1 = assign_roles(team1)
    roles2 = assign_roles(team2)
...
kb = types.InlineKeyboardMarkup()  
kb.add(types.InlineKeyboardButton("✅ Я выиграл", callback_data=f"result_{mode}"))  

for p in players:  
    try:  
        bot.send_message(p, text, reply_markup=kb)  
    except:  
        pass  
...
cap["banned"].append(mp)  

cap["turn"] = cap["team2"] if cap["turn"] == cap["team1"] else cap["team1"]  

remaining = [m for m in MAPS if m not in cap["banned"]]  
...
for m in remaining:  
    kb.add(types.InlineKeyboardButton(  
        f"BAN {m}",  
        callback_data=f"map_{mode}_ban_{m}"  
    ))  

for p in lobbies.get(mode, []):  
    try:  
        bot.send_message(p, "🎯 PICK/BAN ФАЗА", reply_markup=kb)  
    except:  
        pass  
...
if len(remaining) == 1:  
    ...
    for p in lobbies.get(mode, []):  
        bot.send_message(p, f"🗺 FINAL MAP: {final_map}")  

    start_match(mode)  

    captains.pop(mode, None)

=========================================

# AUTO TRIGGER WHEN READY FULL

=========================================

def check_start(mode):

if len(ready_matches.get(mode, [])) >= sizes[mode]:  
